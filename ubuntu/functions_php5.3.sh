...
{
#Install packeages
ubuntu_ver=$(cat /etc/issue | awk '{ print $2 }' | awk -F "." '{print $1}')
if [ "$ubuntu_ver" = "10" ];then
...
if [ $bit = "64" ]; then
apt-get install -y --force-yes -q bzip2 libncurses5-dev ia32-libs bison lemon re2c flex expect libmysql++-dev autoconf2.13 gcc g++ libjpeg62-dev libpng12-dev libxml2-dev curl libcurl4-openssl-dev libmcrypt-dev libmhash-dev libfreetype6-dev patch make mcrypt mysql-server libmysql++-dev zlib-bin zlib1g-dev libtool libltdl*
else
apt-get install -y --force-yes -q bzip2 libncurses5-dev bison lemon re2c flex expect libmysql++-dev autoconf2.13 gcc g++ libjpeg62-dev libpng12-dev libxml2-dev curl libcurl4-openssl-dev libmcrypt-dev libmhash-dev libfreetype6-dev patch make mcrypt mysql-server libmysql++-dev zlib-bin zlib1g-dev libtool libltdl*
fi
...
bit=$(getconf LONG_BIT)
if [ "$bit" == "64" ]; then
apt-get install -y --force-yes -q libncurses5-dev bzip2 ia32-libs bison lemon re2c flex expect autoconf2.13 gcc g++ libjpeg62-dev libpng12-dev libxml2-dev curl libcurl4-openssl-dev libmcrypt-dev libmhash-dev libfreetype6-dev patch make mcrypt libmysql++-dev zlib-bin zlib1g-dev libtool libltdl* libjpeg62
else
apt-get install -y --force-yes -q libncurses5-dev bzip2 bison lemon re2c flex expect autoconf2.13 gcc g++ libjpeg62-dev libpng12-dev libxml2-dev curl libcurl4-openssl-dev libmcrypt-dev libmhash-dev libfreetype6-dev patch make mcrypt libmysql++-dev zlib-bin zlib1g-dev libtool libltdl* libjpeg62
fi
if [ -f /etc/apt/sources.list.bak ];then
mv -f /etc/apt/sources.list.bak /etc/apt/sources.list
fi
...
cd mysql-5.5.27
cmake . -DCMAKE_INSTALL_PREFIX=/usr/local/mysql -DMYSQL_DATADIR=/var/database/mysql -DSYSCONFDIR=/etc/ -DDEFAULT_CHARSET=utf8 -DDEFAULT_COLLATION=utf8_general_ci -DEXTRA_CHARSETS=all -DWITH_MYISAM_STORAGE_ENGINE=1 -DWITH_INNOBASE_STORAGE_ENGINE=1 -DWITH_READLINE=1 -DENABLED_LOCAL_INFILE=1 -DMYSQL_TCP_PORT=3306
...
tar zxf $ubuntutools
mv $ubuntutools_dir/* /root/linuxlmp/
}
...
installed_file()
{
echo "LinuxLMP 0.1 Ubuntu" >> /root/linuxlmp/.installed
}
