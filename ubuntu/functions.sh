...
bit=$(getconf LONG_BIT)
if [ "$bit" == "64" ]; then
apt-get install -y --force-yes -q bzip2 ia32-libs bison lemon re2c flex expect autoconf2.13 gcc g++ libjpeg62-dev libpng12-dev libxml2-dev curl libcurl4-openssl-dev libmcrypt-dev libmhash-dev libfreetype6-dev patch make mcrypt libmysql++-dev zlib-bin zlib1g-dev libtool libltdl* libjpeg62
else
apt-get install -y --force-yes -q bzip2 bison lemon re2c flex expect autoconf2.13 gcc g++ libjpeg62-dev libpng12-dev libxml2-dev curl libcurl4-openssl-dev libmcrypt-dev libmhash-dev libfreetype6-dev patch make mcrypt libmysql++-dev zlib-bin zlib1g-dev libtool libltdl* libjpeg62
fi
if [ -f /etc/apt/sources.list.bak ];then
mv -f /etc/apt/sources.list.bak /etc/apt/sources.list
fi
...
cd /tmp/linuxlmp/php-$php_52_ver
patch -p1 < /tmp/linuxlmp/$php_52_mail_header_patch
mv /tmp/linuxlmp/litespeed /tmp/linuxlmp/php-$php_52_ver/sapi/litespeed/
cd /tmp/linuxlmp
